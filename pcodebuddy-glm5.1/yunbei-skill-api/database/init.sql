...
CREATE TABLE `skills` (
  ...
  `summary` varchar(200) NOT NULL COMMENT '简介（简短描述，不超过100字）',
  `detail` text COMMENT '详细说明（完整说明）',
  `author` varchar(200) NOT NULL COMMENT '作者（文本输入，非账号关联）',
  `category_id` bigint NOT NULL COMMENT '分类ID',
  `source_type` varchar(50) NOT NULL COMMENT '来源类型（internal/external）',
  `source_url` varchar(500) DEFAULT NULL COMMENT '来源网址（外部平台时必填）',
  `source_url_name` varchar(200) DEFAULT NULL COMMENT '来源网址名称（如clawhub.ai）',
  `submitter_id` bigint NOT NULL COMMENT '提交人ID',
  `status` varchar(50) NOT NULL DEFAULT 'pending_review' COMMENT '状态（pending_review/reviewing/approved/published/offline/rejected）',
  `visibility_type` varchar(50) NOT NULL DEFAULT 'all' COMMENT '可见范围类型（all/project/account）',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `skill_versions` (
  ...
  `version_number` varchar(50) NOT NULL COMMENT '版本号（如1.0.0）',
  `zip_path` varchar(500) NOT NULL COMMENT '文件存储路径',
  `zip_size` bigint NOT NULL COMMENT '文件大小（字节）',
  `change_log` text COMMENT '更新日志',
  `uploader_id` bigint NOT NULL COMMENT '上传人ID',
  `status` varchar(50) NOT NULL DEFAULT 'pending_review' COMMENT '状态（pending_review/reviewing/approved/published/offline）',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `ratings` (
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_user_skill_version` (`user_id`, `skill_id`, `version_id`),
  KEY `idx_skill_id` (`skill_id`)
...
CREATE TABLE `feedbacks` (
  ...
  `is_invalid` tinyint NOT NULL DEFAULT 0 COMMENT '是否无效反馈（0=正常，1=管理员标记无效）',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_skill_id` (`skill_id`)
...
CREATE TABLE `notifications` (
  ...
  `type` varchar(50) NOT NULL COMMENT '通知类型（version_update/review_assigned/review_result/review_assigned）',
...
-- 创建管理员账号（密码 admin123 的bcrypt哈希值）
INSERT INTO `users` (`username`, `password`, `real_name`, `department_id`, `status`) VALUES
('admin', '$2b$10$ax1zaUY2BeQmsj8te62TSeGlwlJcXQtfN8IqZFRpVnsv4k7SWkyLa', '系统管理员', 1, 1);
